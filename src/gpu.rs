...
pub struct GPU {
    ...
    // reset the counter.
    ly: u8, //FF44 - LY - LCDC Y-Coordinate (R)
    ...
    lyc: u8, //FF45 - LYC - LY Compare (R/W)
    //--------LCD Monochrome Palettes------------
    // This register assigns gray shades to the color numbers of the BG and Window tiles.
    bgp: u8, //FF47 - BGP - BG Palette Data (R/W) - Non CGB Mode Only
    ...
    obp0: u8, //FF48 - OBP0 - Object Palette 0 Data (R/W) - Non CGB Mode Only
    // This register assigns gray shades for sprite palette 1. It works exactly as BGP (FF47), except that the lower
    // two bits aren't used because sprite data 00 is transparent.
    obp1: u8, //FF49 - OBP1 - Object Palette 1 Data (R/W) - Non CGB Mode Only
    //--------LCD Color Palettes (CGB only)------------
    cbgpi: BGPI, //FF68 - BCPS/BGPI - CGB Mode Only - Background Palette Index
    ...
    cbgpd: [[[u8; 3]; 4]; 8], //FF69 - BCPD/BGPD - CGB Mode Only - Background Palette Data

    cobpi: BGPI,              //FF6A - OCPS/OBPI - CGB Mode Only - Sprite Palette Index
    cobpd: [[[u8; 3]; 4]; 8], //FF6B - OCPD/OBPD - CGB Mode Only - Sprite Palette Data
...
impl GPU {
    ...
    fn read_byte_ram0(&self, a: u16) -> u8 {
        self.ram[a as usize - 0x8000]
    }

    fn read_byte_ram1(&self, a: u16) -> u8 {
    ...
    // 2  Dark gray
    // 3  Black
    fn read_byte_gray_shades(v: u8, i: usize) -> GrayShades {
        match v >> (2 * i) & 0x03 {
    ...
    pub fn next(&mut self, cycles: u32) {
        ...
        for i in 0..c {
            ...
            if d != self.dots {
                ...
                if self.stat.enable_ly_interrupt && self.ly == self.lyc {
                    self.intf.borrow_mut().hi(Flag::LCDCStatus);
                }
            }
            if self.ly >= 144 {
                if self.stat.mode == 1 {
    ...
    fn draw_bg(&mut self) {
        ...
        for x in 0..SCREEN_W {
            ...
            let tile_number = self.read_byte_ram0(tile_addr);
            ...
            let tile_attr = Attr::from(self.read_byte_ram1(tile_addr));

            let tile_y = if tile_attr.yflip { 7 - py % 8 } else { py % 8 };
            let tile_y_data: [u8; 2] = if self.term == Term::GBC && tile_attr.bank {
                let a = self.read_byte_ram1(tile_location + u16::from(tile_y * 2));
                let b = self.read_byte_ram1(tile_location + u16::from(tile_y * 2) + 1);
                [a, b]
            } else {
                let a = self.read_byte_ram0(tile_location + u16::from(tile_y * 2));
                let b = self.read_byte_ram0(tile_location + u16::from(tile_y * 2) + 1);
                [a, b]
            };
            ...
            if self.term == Term::GBC {
                ...
                let color = Self::read_byte_gray_shades(self.bgp, color) as u8;
    ...
    fn draw_sprites(&mut self) {
        ...
        for i in 0..40 {
            ...
            let py = self.read_byte(sprite_addr).wrapping_sub(16);
            let px = self.read_byte(sprite_addr + 1).wrapping_sub(8);
            let tile_number =
                self.read_byte(sprite_addr + 2) & if self.lcdc.bit2() { 0xfe } else { 0xff };
            ...
            let tile_y_data: [u8; 2] = if self.term == Term::GBC && tile_attr.bank {
                let b1 = self.read_byte_ram1(tile_y_addr);
                let b2 = self.read_byte_ram1(tile_y_addr + 1);
                [b1, b2]
            } else {
                let b1 = self.read_byte_ram0(tile_y_addr);
                let b2 = self.read_byte_ram0(tile_y_addr + 1);
                [b1, b2]
            };
            ...
            for x in 0..8 {
                ...
                if self.term == Term::GBC {
                    ...
                    let color = if tile_attr.palette_number_0 == 1 {
                        Self::read_byte_gray_shades(self.obp1, color) as u8
                    } else {
                        Self::read_byte_gray_shades(self.obp0, color) as u8
                    };
