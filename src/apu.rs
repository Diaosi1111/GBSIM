...
impl Memory for ChannelSquare {
    ...
    fn write_byte(&mut self, addr: u16, val: u8) {
        match addr {
            0xff10 | 0xff15 => self.reg.borrow_mut().nrx0 = val,
            0xff11 | 0xff16 => {
                self.reg.borrow_mut().nrx1 = val;
                self.lc.n = self.reg.borrow().get_length_load();
            }
            0xff12 | 0xff17 => self.reg.borrow_mut().nrx2 = val,
            0xff13 | 0xff18 => {
                self.reg.borrow_mut().nrx3 = val;
                self.timer.period = period(self.reg.clone());
            }
            0xff14 | 0xff19 => {
                self.reg.borrow_mut().nrx4 = val;
                self.timer.period = period(self.reg.clone());
                // Trigger Event
                //
...
impl Memory for ChannelWave {
    fn read_byte(&self, addr: u16) -> u8 {
        match addr {
            ...
            0xff30..=0xff3f => self.waveram[addr as usize - 0xff30],
            _ => unreachable!(),
        }
    }
    ...
    fn write_byte(&mut self, addr: u16, val: u8) {
        match addr {
            0xff1a => self.reg.borrow_mut().nrx0 = val,
            0xff1b => {
                self.reg.borrow_mut().nrx1 = val;
                self.lc.n = self.reg.borrow().get_length_load();
            }
            0xff1c => self.reg.borrow_mut().nrx2 = val,
            0xff1d => {
                self.reg.borrow_mut().nrx3 = val;
                self.timer.period = period(self.reg.clone());
            }
            ...
            0xff30..=0xff3f => self.waveram[addr as usize - 0xff30] = val,
            _ => unreachable!(),
        }
    }
}
...
impl Memory for ChannelNoise {
    ...
    fn write_byte(&mut self, addr: u16, val: u8) {
        match addr {
            0xff1f => self.reg.borrow_mut().nrx0 = val,
            0xff20 => {
                self.reg.borrow_mut().nrx1 = val;
                self.lc.n = self.reg.borrow().get_length_load();
            }
            0xff21 => self.reg.borrow_mut().nrx2 = val,
            0xff22 => {
                self.reg.borrow_mut().nrx3 = val;
                self.timer.period = period(self.reg.clone());
            }
...
impl Memory for Apu {
    fn read_byte(&self, addr: u16) -> u8 {
        let r = match addr {
            ...
            0xff15..=0xff19 => self.channel2.read_byte(addr),
            0xff1a..=0xff1e => self.channel3.read_byte(addr),
            0xff1f..=0xff23 => self.channel4.read_byte(addr),
        ...
        r | RD_MASK[addr as usize - 0xff10]
    }

    fn write_byte(&mut self, addr: u16, val: u8) {
        if addr != 0xff26 && !self.reg.get_power() {
            return;
        }
        match addr {
            0xff10..=0xff14 => self.channel1.write_byte(addr, val),
            0xff15..=0xff19 => self.channel2.write_byte(addr, val),
            0xff1a..=0xff1e => self.channel3.write_byte(addr, val),
            0xff1f..=0xff23 => self.channel4.write_byte(addr, val),
            0xff24 => self.reg.nrx0 = val,
            0xff25 => self.reg.nrx1 = val,
            0xff26 => {
                self.reg.nrx2 = val;
